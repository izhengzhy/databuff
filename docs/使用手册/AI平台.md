<p align="center">
  <a href="AI平台.md">中文</a>
  &nbsp;|&nbsp;
  <a href="AI平台_en.md">English</a>
</p>

# 使用手册 · AI 平台

## 这是什么

平台的**智能大脑** —— 用对话代替翻图表，用 AI 代替人工排查。

你问，它查数据、做分析、给结论。

---

## 快速上手

1. **配置模型**：配置管理 → 模型配置 → 填入 API Key
2. **开始对话**：AI 平台 → 输入问题
3. **拿到答案**：AI 自动查数据并回复

---

## 数字专家

```mermaid
flowchart LR
  Brain["AI 大脑<br/>理解 + 分派"]
  Data["智能问数<br/>查数据"]
  Insp["巡检专家<br/>找异常"]
  Qa["产品答疑<br/>怎么用"]
  Brain --> Data
  Brain --> Insp
  Brain --> Qa
```

| 专家 | 擅长 | 举例 |
|------|------|------|
| **AI 大脑** | 理解你的问题，派给对的专家 | 「payment 服务最近怎么样？」 |
| **智能问数** | 查指标、Trace、拓扑、告警 | 「order 服务 QPS 趋势」 |
| **巡检专家** | 主动发现服务异常 | 「巡检一下 inventory 服务」 |
| **产品答疑** | 产品怎么用、功能说明、配置含义 | 「告警规则在哪配？」 |

> 日常直接跟 **AI 大脑** 对话即可，它会自动路由。

---

## 能帮你做什么

| 场景 | 你可以这样问 |
|------|-------------|
| 看服务健康 | 「哪些服务错误率最高？」 |
| 查性能趋势 | 「order 服务最近 1 小时延迟怎么样？」 |
| 追慢请求 | 「找 5 条最慢的 Trace」 |
| 理清依赖 | 「payment 服务调了谁、被谁调？」 |
| 主动巡检 | 「帮我巡检所有核心服务」 |
| 故障定位 | 「order 错误率突然升高，什么原因？」 |

---

## 多智能体协同

复杂问题不用你拆解 —— **AI 大脑会自动派多个专家并行处理**，最后汇总给你一份完整结论。

```mermaid
flowchart TD
  Q["order 服务异常，帮我全面分析"] --> Brain["AI 大脑"]
  Brain --> A["问数专家：查指标和 Trace"]
  Brain --> B["巡检专家：健康扫描"]
  A & B --> Answer["综合诊断报告"]
```

这就是 DataBuff 区别于「简单聊天机器人」的核心：**不是陪你聊天，是帮你干活**。

---

## 扩展外部能力

需要扩展平台能力边界？见 [自定义数字专家](自定义数字专家.md)——注册 MCP、创建 SPECIALIST 专家、由 AI 大脑自动路由派发。

需要把 GitHub、浏览器、知识库等外部系统接进 AI 排障流程？见 [外部 MCP 集成](外部MCP集成.md)——在工具管理里注册远程 MCP，绑定到数字专家，即可在 AI 对话中调用。
