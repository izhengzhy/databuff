...
- **FE**：`docker-compose.yml` 启动前 `sed` 将 `-Xmx8192m` 改为 `-Xmx1200m`，与 2 GiB limit 匹配。**勿删除**该 patch。
- **BE OOM**：按下面步骤上调 BE 容器内存上限。

**如何上调 Doris BE 内存（Docker）：**

1. `cd /opt/databuff-ai-apm && cp docker-compose.yml docker-compose.yml.bak`
2. 找到 `ai-apm-doris-be`，将 `mem_limit: 6g` 改为 `8g`（宿主机需有足够空闲内存）。
3. `docker compose up -d ai-apm-doris-be`
4. `docker stats ai-apm-doris-be --no-stream` 观察 MEM USAGE。

![修改 BE mem_limit 示例](../images/perf-docker-be-memory.png)

若仍 OOM，优先 **缩短 `dynamic_partition.start`** 或扩容磁盘，再加内存。
...
| Doris BE OOM / 重启 | BE `mem_limit`、保留天数、`be-storage` 使用率 | 加内存或 `ALTER TABLE` 缩短 `dynamic_partition.start` |
